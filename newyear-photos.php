...
$que="select * from love_images where image_category='New_year_Photos'"; 
$run=mysqli_query($con,$que);
   if(mysqli_num_rows($run) >= 1)
     {
	   $recordset = mysqli_query($con,"select * from love_images where image_category='New_year_Photos'");
		 ...
		 if(!empty($record["image_name"]))
	       {
	         $url= "images/New_year_Photos/".$record["image_name"];
	       }

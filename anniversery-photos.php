...
$que="select * from love_images where image_category='Sad_Photos'"; 
$run=mysqli_query($con,$que);
   if(mysqli_num_rows($run) >= 1)
     {
	   $recordset = mysqli_query($con,"select * from love_images where image_category='Anniversery_Photos'");
		 ...
		 if(!empty($record["image_name"]))
	       {
	         $url= "images/Anniversery_Photos/".$record["image_name"];
	       }

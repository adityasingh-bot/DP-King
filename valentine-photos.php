...
$que="select * from love_images where image_category='Valentine_Photos'"; 
$run=mysqli_query($con,$que);
   if(mysqli_num_rows($run) >= 1)
     {
	   $recordset = mysqli_query($con,"select * from love_images where image_category='Valentine_Photos'");
		 ...
		 if(!empty($record["image_name"]))
	       {
	         $url= "images/Valentine_Photos/".$record["image_name"];
	       }
